 ...
 */
static void kthread_add_sab(struct plist_node *node, struct plist_head *head)
{
 ...
 */
static void kthread_del(struct plist_node *node, struct plist_head *head)
{
	plist_check_head(head);
...
static struct task_struct *oplus_detach_running_task_for_rt(struct lb_env *env)
{
	...
	plist_for_each_entry(p, head, pushable_tasks) {
		...
		trace_printk("DEBUG_LB_RT_TICK[%d]: p=%s$%d, lb.pid=%d "
			...
			READ_ONCE(p->state), p->on_cpu, p->on_rq, task_cpu(p));
#endif
...
static int oplus_active_load_balance_cpu_stop_for_rt(void *data)
{
	...
	if (task) {
		trace_printk("DEBUG_LB_RT_TICK[%d]: task=%s$%d, state=%d, on_cpu=%d, on_rq=%d, task_cpu=%d\n",
			__LINE__, task->comm, task->pid,
			READ_ONCE(task->state), task->on_cpu, task->on_rq, task_cpu(task));
	}
#endif
...
void add_rt_boost_task(struct task_struct *p)
{
	...
	kthread_del(&ots->rtb, &rt_boost_task);
	plist_node_init(&ots->rtb, prio);
	kthread_add_sab(&ots->rtb, &rt_boost_task);
			...
			struct rq *rq, bool rt_boost)
{
	...
	trace_printk("DEBUG_LB_RT_TICK[%d]: curr=%s$%d, "
		...
		"rt_boost=%d, ret=%d\n",
		__LINE__, curr->comm, curr->pid,
		READ_ONCE(curr->state), curr->on_cpu, curr->on_rq, task_cpu(curr),
...
#if !IS_ENABLED(CONFIG_ARCH_QCOM) && !IS_ENABLED(CONFIG_MTK_NEWIDLE_BALANCE)
	/*
	 * for newidle balance.
