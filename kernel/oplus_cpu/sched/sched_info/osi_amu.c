...
pid_t global_pid;
DEFINE_PER_CPU(u32, old_pid_osi);
...
static void amu_sched_switch_handler(void *unused, bool preempt, struct task_struct *prev, struct task_struct *next)
{
	...
	if (ots && (per_cpu(old_pid_osi, cpu) != -1) && (cpu == smp_processor_id())) {
	...
	per_cpu(old_pid_osi, cpu) = cur_pid;
}
...
int osi_amu_init(struct proc_dir_entry *pde)
{
	...
	if (is_support_amu) {
		...
		REGISTER_TRACE_RVH(android_vh_commit_creds, android_vh_commit_creds_handler);
...
int osi_amu_exit(struct proc_dir_entry *pde)
{
	...
	if (is_support_amu) {
		...
		UNREGISTER_TRACE_RVH(android_vh_commit_creds, android_vh_commit_creds_handler);
