...
/**
 * plist_del_osi - Remove a @node from plist.
 *
 * @node:	&struct plist_node pointer - entry to be removed
 * @head:	&struct plist_head pointer - list head
 */
void plist_del_osi(struct plist_node *node, struct plist_head *head)
{
...
int find_in_plist(struct task_struct *p,  struct oplus_task_struct *ots)
{
	...
	if (is_find) {
		plist_del_osi(&tmp->node, &hot_thread_head);
...
static void  get_hot_thread(u32 now_idx, u64 now)
{
	...
	plist_for_each_entry_safe(hot_node, tmp, &hot_thread_head, node) {
		...
		plist_del_osi(&hot_node->node, &hot_thread_head);
