...
struct list_head cluster_head_frame_group;
#define for_each_sched_cluster(cluster) \
	list_for_each_entry_rcu(cluster, &cluster_head_frame_group, list)
...
static void cleanup_clusters(struct list_head *head)
{
	...
	list_for_each_entry_safe(cluster, tmp, head, list) {
		...
		num_sched_clusters_frame_group--;
		kfree(cluster);
	}
}
...
static inline void add_cluster(const struct cpumask *cpus, struct list_head *head)
{
	...
	fb_cluster[num_sched_clusters_frame_group] = cluster;

	num_sched_clusters_frame_group++;
}

static inline void assign_package_ids(struct list_head *head)
{
...
static bool build_clusters(void)
{
	...
	INIT_LIST_HEAD(&cluster_head_frame_group);
	INIT_LIST_HEAD(&new_head);

	/* If this work failed, our cluster_head_frame_group can still used with only one cluster struct */
	for_each_cpu(i, &cpus) {
	...
	assign_package_ids(&new_head);
	move_list(&cluster_head_frame_group, &new_head);
	return true;
}
...
static struct oplus_sched_cluster *best_cluster(struct frame_group *grp)
{
	 ...
	 */
	if (num_sched_clusters_frame_group <= 2) {
		grp->available_cluster = NULL;
	} else {
		if (fb_cluster[num_sched_clusters_frame_group-1]->id == best_cluster->id) {
			/* if best_cluster is cpu7, then available_cluster is cpu4-6 */
			grp->available_cluster = fb_cluster[num_sched_clusters_frame_group-2];

		} else if (fb_cluster[num_sched_clusters_frame_group-2]->id == best_cluster->id) {
			/* if best_cluster is cpu4-6, then available_cluster is cpu7 */
			grp->available_cluster = fb_cluster[num_sched_clusters_frame_group-1];

		} else if (fb_cluster[num_sched_clusters_frame_group-3]->id == best_cluster->id) {
...
int frame_group_init(void)
{
	...
	for_each_sched_cluster(cluster)
		ofb_debug("num_cluster=%d id=%d cpumask=%*pbl capacity=%lu num_cpus=%d\n",
			num_sched_clusters_frame_group, cluster->id, cpumask_pr_args(&cluster->cpus),
